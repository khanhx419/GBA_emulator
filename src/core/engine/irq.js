...
GameBoyAdvanceInterruptHandler.prototype.swi = function(opcode) {
	...
	case 0x1F:
		...
		break;
	default:
		this.core.WARN("Unimplemented software interrupt: 0x" + opcode.toString(16));
		break;
	}
};
...
GameBoyAdvanceInterruptHandler.prototype.halt = function() {
	if (!this.enable) {
		this.core.WARN("Requested HALT when interrupts were disabled!");
		return;
	}
	if (!this.waitForIRQ()) {
		this.core.WARN("Waiting on interrupt forever.");
		return;
	}
}
