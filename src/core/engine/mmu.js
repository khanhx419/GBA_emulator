...
GameBoyAdvanceMMU.prototype.clear = function() {
	...
	if (!this.bios) {
		this.bios = new BIOSView(new ArrayBuffer(this.SIZE_BIOS));
		this.bios.real = false;
	}
	this.memory = [
		this.bios,
...
GameBoyAdvanceMMU.prototype.loadBios = function(bios, real) {
	...
	this.bios.real = !!real;
	if (this.memory) {
		this.memory[this.REGION_BIOS] = this.bios;
	}
};
...
GameBoyAdvanceMMU.prototype.loadRom = function(rom, process) {
	...
	if (lo.view.byteLength > 0xB2 && lo.view.getUint8(0xB2) != 0x96) {
		this.core.WARN('ROM header byte 0xB2 is not standard 0x96 (found 0x' + lo.view.getUint8(0xB2).toString(16) + '), loading anyway');
	}
...
GameBoyAdvanceMMU.prototype.addressToPage = function(region, address) {
	var block = this.memory[region] || this.badMemory;
	var bits = block.ICACHE_PAGE_BITS || 8;
	return address >> bits;
};

GameBoyAdvanceMMU.prototype.accessPage = function(region, pageId) {
	var memory = this.memory[region] || this.badMemory;
	if (!memory.icache) {
		memory.icache = new Array(1);
	}
	var page = memory.icache[pageId];
	var bits = memory.ICACHE_PAGE_BITS || 8;
	if (!page || page.invalid) {
		page = {
			thumb: new Array(1 << bits),
			arm: new Array(1 << (bits - 1)),
			invalid: false
		};
...
GameBoyAdvanceMMU.prototype.serviceDma = function(number, info) {
		...
		if (sourceView && destView) {
			if (width == 4) {
				...
				while (wordsRemaining--) {
					...
					destView.setInt32(dest & destMask, word, true);
					source += sourceOffset;
					dest += destOffset;
				}
			} else {
				while (wordsRemaining--) {
					...
					destView.setUint16(dest & destMask, word, true);
					source += sourceOffset;
					dest += destOffset;
				}
			}
	...
	if (!info.repeat || info.timing == this.DMA_TIMING_NOW) {
		info.enable = false;
		...
		if (info.timing == this.DMA_TIMING_CUSTOM) {
...
export { GameBoyAdvanceMMU, MemoryBlock, ROMView, BIOSView };
