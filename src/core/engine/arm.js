...
ARMCoreArm.prototype.constructAddressingMode23Immediate = function(instruction, immediate, condOp) {
	...
	var idx = (instruction & 0x01A00000) >> 21;
	var fn = this.addressingMode23Immediate[idx];
	...
	if (p) {
		if (w) {
			address = function() {
				var addr = u ? (gprs[rn] + immediate) : (gprs[rn] - immediate);
			...
			address = function() {
				return u ? (gprs[rn] + immediate) : (gprs[rn] - immediate);
			};
			address.writesPC = false;
		}
	} else {
		address = function() {
			var addr = gprs[rn];
			if (!condOp || condOp()) {
				gprs[rn] = u ? (gprs[rn] + immediate) : (gprs[rn] - immediate);
			}
			return addr;
		};
		address.writesPC = rn == cpu.PC;
...
ARMCoreArm.prototype.constructAddressingMode23Register = function(instruction, rm, condOp) {
	var rn = (instruction & 0x000F0000) >> 16;
	var idx = (instruction & 0x01A00000) >> 21;
	var fn = this.addressingMode23Register[idx];
	if (typeof fn === 'function') {
	...
	if (p) {
		if (w) {
			address = function() {
				var addr = u ? (gprs[rn] + gprs[rm]) : (gprs[rn] - gprs[rm]);
				if (!condOp || condOp()) gprs[rn] = addr;
				return addr;
			};
			address.writesPC = rn == cpu.PC;
			...
			address = function() {
				return u ? (gprs[rn] + gprs[rm]) : (gprs[rn] - gprs[rm]);
			};
			address.writesPC = false;
		}
	} else {
		address = function() {
			var addr = gprs[rn];
			if (!condOp || condOp()) {
				gprs[rn] = u ? (gprs[rn] + gprs[rm]) : (gprs[rn] - gprs[rm]);
			}
			return addr;
		};
		address.writesPC = rn == cpu.PC;
...
ARMCoreArm.prototype.constructAddressingMode2RegisterShifted = function(instruction, shiftOp, condOp) {
	var rn = (instruction & 0x000F0000) >> 16;
	var idx = (instruction & 0x01A00000) >> 21;
	var fn = this.addressingMode2RegisterShifted[idx];
	if (typeof fn === 'function') {
	...
	if (p) {
		if (w) {
			address = function() {
				shiftOp();
				var addr = u ? (gprs[rn] + cpu.shifterOperand) : (gprs[rn] - cpu.shifterOperand);
				if (!condOp || condOp()) gprs[rn] = addr;
				return addr;
			};
			address.writesPC = rn == cpu.PC;
			...
			address = function() {
				shiftOp();
				return u ? (gprs[rn] + cpu.shifterOperand) : (gprs[rn] - cpu.shifterOperand);
			};
			address.writesPC = false;
		}
	} else {
		address = function() {
			...
			var addr = gprs[rn];
			if (!condOp || condOp()) {
				gprs[rn] = u ? (gprs[rn] + cpu.shifterOperand) : (gprs[rn] - cpu.shifterOperand);
			}
			return addr;
		};
		address.writesPC = rn == cpu.PC;
