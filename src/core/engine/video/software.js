...
GameBoyAdvanceVRAM.prototype.loadU8 = function(offset) {
	offset %= 0x18000;
	var index = offset >> 1;
	if (offset & 1) {
		return (this.buffer[index] & 0xFF00) >>> 8;
...
GameBoyAdvanceVRAM.prototype.loadU16 = function(offset) {
	return this.buffer[(offset % 0x18000) >> 1];
};

GameBoyAdvanceVRAM.prototype.load16 = function(offset) {
	return (this.loadU16(offset) << 16) >> 16;
};
...
GameBoyAdvanceVRAM.prototype.load32 = function(offset) {
	offset %= 0x18000;
	var idx = offset >> 1;
	return this.buffer[idx & ~1] | (this.buffer[idx | 1] << 16);
};
...
GameBoyAdvanceVRAM.prototype.store16 = function(offset, value) {
	this.buffer[(offset % 0x18000) >> 1] = value;
};

GameBoyAdvanceVRAM.prototype.store32 = function(offset, value) {
	offset %= 0x18000;
	var index = offset >> 1;
	this.buffer[index] = value & 0xFFFF;
	this.buffer[index + 1] = value >>> 16;
};
...
GameBoyAdvancePalette.prototype.convert16To32 = function(value, input) {
	var r = value & 0x001F;
	var g = (value & 0x03E0) >> 5;
	var b = (value & 0x7C00) >> 10;
...
GameBoyAdvanceSoftwareRenderer.prototype.writeDisplayControl = function(value) {
	...
	this.bg[2].multipalette = this.bg[2].rawMultipalette;
	this.bg[3].multipalette = this.bg[3].rawMultipalette;
...
GameBoyAdvanceSoftwareRenderer.prototype.writeBackgroundControl = function(bg, value) {
	...
	bgData.rawMultipalette = value & 0x0080;
	bgData.multipalette = bgData.rawMultipalette;
	if (bg == 2 && this.backgroundMode > 0) {
		bgData.multipalette |= 0x0001;
	} else if (bg == 3 && this.backgroundMode == 2) {
		bgData.multipalette |= 0x0001;
	}
...
GameBoyAdvanceSoftwareRenderer.prototype.drawScanlineBGMode0 = function(backing, bg, start, end) {
	...
	var yMask = (size & 2) ? 0x1FF : 0xFF;
	var xMask = (size & 1) ? 0x1FF : 0xFF;
	var localY = (y + yOff) & yMask;
	if (this.mosaic) {
		localY = (localY - (y % video.bgMosaicY) + 512) & yMask;
	}
	var localYLo = localY & 0x7;
	...
	var tileRow = 0;
	var lastFetchMapX = -1;
	var lastFetchTileX = -1;

	for (x = start; x < end; ++x) {
		localX = (x + xOff) & xMask;
		if (this.mosaic) {
			mosaicX = offset % video.bgMosaicX;
			localX = (localX - mosaicX + 512) & xMask;
		}
		localXLo = localX & 0x7;
		var tileX = localX >> 3;

		if (tileX !== lastFetchMapX) {
			lastFetchMapX = tileX;
			video.accessMapMode0(screenBase, size, localX, yBase, map);
			lastFetchTileX = -1;
		}

		var tileRowIdx = !map.vflip ? localYLo : (7 - localYLo);

		if (!paletteShift) {
			if (lastFetchTileX !== tileX) {
				lastFetchTileX = tileX;
				tileRow = video.accessTile(charBase, map.tile, tileRowIdx);
			}
			var pixelX = map.hflip ? (7 - localXLo) : localXLo;
			bg.pushPixel(index, map, video, tileRow, pixelX, offset, backing, mask, false);
		} else {
			var pixelX = map.hflip ? (7 - localXLo) : localXLo;
			var wordIdx = pixelX >> 2;
			var fetchKey = (tileX << 1) | wordIdx;
			if (lastFetchTileX !== fetchKey) {
				lastFetchTileX = fetchKey;
				tileRow = video.accessTile(charBase + (wordIdx << 2), map.tile << 1, tileRowIdx << 1);
			}
			bg.pushPixel(index, map, video, tileRow, pixelX & 0x3, offset, backing, mask, false);
		}
		offset++;
	}
};
...
GameBoyAdvanceSoftwareRenderer.prototype.drawScanline = function(y) {
	...
	var hasWindows = this.win0 || this.win1 || this.objwin;
	var segStarts = [0];
	var segWindows = [];

	if (hasWindows) {
		var inWin0Y = this.win0 && (this.win0Top <= this.win0Bottom ? (y >= this.win0Top && y < this.win0Bottom) : (y < this.win0Bottom || y >= this.win0Top));
		var inWin1Y = this.win1 && (this.win1Top <= this.win1Bottom ? (y >= this.win1Top && y < this.win1Bottom) : (y < this.win1Bottom || y >= this.win1Top));

		var curWin = -1;
		for (var px = 0; px < this.HORIZONTAL_PIXELS; ++px) {
			var w = 2; // Default to WINOUT
			if (inWin0Y && (this.win0Left <= this.win0Right ? (px >= this.win0Left && px < this.win0Right) : (px < this.win0Right || px >= this.win0Left))) {
				w = 0;
			} else if (inWin1Y && (this.win1Left <= this.win1Right ? (px >= this.win1Left && px < this.win1Right) : (px < this.win1Right || px >= this.win1Left))) {
				w = 1;
			}
			if (w !== curWin) {
				if (curWin !== -1) {
					segStarts.push(px);
					segWindows.push(curWin);
				}
				curWin = w;
			}
		}
		segStarts.push(this.HORIZONTAL_PIXELS);
		segWindows.push(curWin);
	}
	...
	for (var i = 0; i < this.drawLayers.length; ++i) {
		...
		if (!hasWindows) {
			...
			for (var s = 0; s < segWindows.length; ++s) {
				var winId = segWindows[s];
				var sStart = segStarts[s];
				var sEnd = segStarts[s + 1];
				...
				if (winId === 0) {
					if (this.windows[0].enabled[layer.index]) {
						...
						layer.drawScanline(backing, layer, sStart, sEnd);
					}
				} else if (winId === 1) {
...
export { GameBoyAdvanceSoftwareRenderer };
